...
const countSentences = (str)=>{
      let regex = /[!\.\?]/
      str = str.trim()
      let lastPunctuation =  str[str.length-1]
      let counter = (lastPunctuation === '!' || lastPunctuation === '.'  || lastPunctuation === '?' )? 0 : 1
      let textSplit = str.split(regex)
      let sentenceArray = textSplit.filter(sentence => sentence )
      return sentenceArray.length-counter
}